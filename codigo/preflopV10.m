function [FICHAS,aportes_juego,numero_tres,fold_A,opcion,all_in,orden_jugadores,botes,jug_AI,ALL_IN]=preflopV10(FICHAS,jugada,ronda,cartas_computadora1,cartas_computadora2,mapacartas)

%INICIALIZACION DE VARIABLES --------------------------------------------------------------------------
aportes_juego=[0,0,0]; %Inicializar aportes de cada juego <<NO SE AGREGA)>>
aportes_ronda=[0,0,0]; %Inicializar aportes de cada ronda <<SE AGREGA con 0,0,0)>>
apuesta_actual=10; %La apuesta inicial es la ciega grande <<SE AGREGA con 0 para flop, turn y river>>
ronda_completa=false; %Inicializar ronda (false para iniciar el ciclo while) <<SE AGREGA>>
opcion=[1 1]; %Inicializar primer y segunda apuesta (ciega pequeña y ciega grande) <<NO SE AGREGA)>>
fold_A=[1,1,1]; %Inicializar valores logicos para controlar el retiro de cada jugador <<NO SE AGREGA)>>
all_in=[0,0,0]; %Inicializar valores logicos para controlar all in de cada jugador <<NO SE AGREGA)>>
ALL_IN=[0,0,0]; %Inicializar valores logicos para controlar ALL IN de cada jugador <<NO SE AGREGA)>>
botes=0; %Inicializar cantidad de botes paralelos <<NO SE AGREGA)>>
jug_AI=0; %Inicializar cantidad de jugadores en ALL IN (supremo) <<NO SE AGREGA)>>
...
%[PARTE 1] Determinar el orden de los jugadores en función de "jugada", determinar y mostrar a quien le toca la ciega pequeña y grande <<NO SE AGREGA)>>
[orden_jugadores,aportes_juego,aportes_ronda,FICHAS]=orden_y_ciegas(jugada,FICHAS,aportes_juego,aportes_ronda);
...
while ronda_completa==false
    ...
    for k=1:3 %Orden de jugadores
        ...
        if i==1 && fold_A(i)==1 && FICHAS(i)>0 && all_in(i)==0 && botes~=3 && jug_AI~=2
            [apuesta_actual,aportes_ronda,FICHAS,aportes_juego,all_in,fold_A,opcion,ALL_IN]=manoplaV10(i,apuesta_actual,aportes_ronda,FICHAS,aportes_juego,all_in,fold_A,opcion,ALL_IN,ronda,cartas_computadora1,cartas_computadora2,mapacartas);
